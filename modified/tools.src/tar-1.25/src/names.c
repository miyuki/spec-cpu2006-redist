...
#if defined(WIN32)
#include <windows.h>
 /* This definition of ERROR comes from paxlib.h; wingdi.h (included from
  * windows.h) nukes it
  */
# undef ERROR
# define ERROR(Args) (error Args, exit_status = PAXEXIT_FAILURE)
struct group {
...
/* getpwuid modified from the version in mingw.c in git -- also under GPL */
struct passwd {
         uid_t pw_uid;
         char *pw_name;
         char *pw_gecos;
