...
{
#if defined(WIN32)
  /* Leak some file descriptors, but only on Windows.
   * The call to fdopendir() quite effectively closes st->fd.  The GNUlib
   * fd<->name mapping stuff deallocates the entry, and subsequent calls to
   * GNUlib's fchdir (via openat) on child files fails miserably.
   * So dup() the current file descriptor (which ensures that it will have
   * a name mapping for fchdir).
   */
  int tmpfd = dup(st->fd);
#endif
  while (! (st->dirstream = fdopendir (st->fd)))
    if (! open_failure_recover (st))
      return 0;
#if defined(WIN32)
  /* Plug in our duplicate for future use... */
  st->fd = tmpfd;
#endif
...
{
#if defined(WIN32)
	/* st_dev is (drive letter - 'A') and st_ino is always 0 on Windows
	 * filesystems, so files on the same drive always hash together.
	 * The quick-n-dirty solution for SPEC: dump all files separately.
	 */
#else
  if (link_table
...
{
#if defined(WIN32)
	/* See comments in dump_hard_link() about st_ino on Windows. */
	return;
#else
