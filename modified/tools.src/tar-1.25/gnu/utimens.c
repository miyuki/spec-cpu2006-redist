...
#else
      /* Look at the return code, and if it's EACCES check to see if file is
       * a directory.  If it is, ignore the error and return success.
       */
      rc = utime (file, ut);
      if (rc < 0 && errno == EACCES) {
        if (_stat (file, &statbuf) >= 0) {
          if (statbuf.st_mode & _S_IFDIR)
            /* Can't do utime on a directory, so just ignore it. */
            return 0;
        }
        errno = EACCES; /* Just putting it back the way it was... */
      }
      return rc;
#endif
    }
