 ...
 */
/*
 * Modified by Cloyce D. Spradling for use with benchmarks from the
...
static const char helpstring[] =
    ...
    "\n-spec\tGenerate dependencies suitable for use with SPEC benchmarks\n"
    "\n\nReport bugs to support@spec.org\n";
...
static const char license[]=
    "\nCopyright (C) 2000--2006 Erik Edelmann <Erik.Edelmann@iki.fi>\n"
    "Modifications for SPEC benchmarks Copyright 2004-2010 Standard Performance\n"
    "   Evaluation Corporation <info@spec.org>\n"
    "\n"
...
int main (int argc, char **argv)
{
    ...
    Module *mod;
    List *h1, *h2, *targets;
    char **seen = NULL;        /* To not print duplicates */
    int seencount = 0, found = 0;
    int indent, itemindent;
    ...
    for (i = 1; i < argc; i++) {
         ...
         */
            ...
            options.spec = true;
            options.src_dep = false;
	    options.obj_dir = "\0"; /* Maybe leak some memory */
	    options.coco = false;
	    options.create_obj = false;
	    options.modfile_fmt = "%s";
    ...
    if (options.spec) {                 /* The SPEC action is here */
      printf ("%%deps = (\n");
      indent = 10;
      itemindent = 0;
      for (h1 = deplist; h1; h1 = h1->next) {
        ...
        /* If there are no dependencies, there is no need to output anything */
        if (list_length(dep->includes) + list_length(dep->modules) == 0)
	  continue;
        else {
          /* Allocate memory for the seen list */
          seen = (char **)xmalloc(sizeof(char **) * (list_length(dep->includes) + list_length(dep->modules)));
          seencount = 0;
        }


        /* Targets */
        for (targets = dep->targets; targets; targets = targets->next) {
	  target_printed = false;
	  /* Wait to print the target name, since there may be no deps */
	  itemindent = strlen(targets->data) + 8;
	  indent += itemindent;

	  /* Includes */
	  for (h2 = dep->includes; h2; h2 = h2->next) {
            /* Check that the current file isn't in the 'seen' list */
            for (i = 0; i < seencount; i++) {
              if (!strcmp(seen[i], (char *)h2->data)) {
                found = 1;
                break;
              }
            }
	    ...
	    if (!target_printed) {
	      printf ("%*s", indent - itemindent, " ");
              if (options.obj_dir_set)
                  printf("'%s' => [\n", set_path(targets->data, options.obj_dir));
              else
                  printf("'%s' => [\n", (char *)targets->data);
	      target_printed = true;
	    }
	    printf ("%*s'%s',\t# include\n", indent, " ", (char *)h2->data);
            /* Remember it */
            seen[seencount++] = xstrdup(h2->data);
	  }
	  ...
	  for (h2 = dep->modules; h2; h2 = h2->next) {
	    ...
	    if (!(l = list_find (modlist, s, &modstrcmp))) {
	      int tmplen = 0;
              char *tmp = NULL;
	      /* Warn about an undefined use, but write it out anyway */
	      if (options.warn_missing)  warning ("Module '%s' not found", s);

	      /* Guess that the name of the module's source file will be the
	       * same as the name of the module, with the extension of the
	       * original file.  So that a human will check, add an "XXX"
	       * in a place where it'll cause a syntax error.
	       */
              tmplen = strlen(s) + strlen(get_suffix(targets->data)) + 1024;
              tmp = xmalloc(tmplen);
              strncpy(tmp, s, tmplen);
              strcat(tmp, get_suffix(targets->data));
	      if (!target_printed) {
                printf ("%*s", indent - itemindent, " ");
                if (options.obj_dir_set)
                    printf("'%s' => [\n", set_path(targets->data, options.obj_dir));
		...
		target_printed = true;
	      ...
	      printf ("[XXX CHECK]%*s'%s',\t# (module)\n", indent, " ", tmp);
              /* Remember it */
              seen[seencount++] = xstrdup(tmp);
	      free(tmp);
	    } else {
	      mod = (Module *)l->data;

              for (i = 0; i < seencount; i++) {
                if (!strcmp(seen[i], mod->modfile_name)) {
                  found = 1;
                  break;
                }
              }
		...
		/* Don't write *.mod-file to the dependency list if it 
		...
		printf ("%*s", indent, " ");
                if (options.obj_dir_set)
                    ...
                    printf("'%s',\t# module\n", mod->modfile_name);
                /* Remember it */
                seen[seencount++] = xstrdup(mod->modfile_name);
              }
	    }
	  ...
	  for (h2 = extradeps; h2; h2 = h2->next) {
	    ...
	    printf ("%*s'%s',\t# explicit\n", indent, " ", (char *)h2->data);
          ...
          /* -R and -r don't apply when using -spec */
