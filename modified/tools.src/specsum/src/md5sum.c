...
split_3 (char *s, size_t s_len,
         unsigned char **hex_digest, int *binary, size_t *size, char **file_name)
{
  ...
  *hex_digest = (unsigned char *) &s[i];
  ...
  if (s[i] != ' ' && s[i] != '*')
    return false;
  /* For the purposes of SPEC CPU, there are NO non-binary files */
  *binary = (s[i++] == '*') || 1;
...
static bool
digest_file (const char *filename, int *binary, size_t *size, unsigned char *bin_result)
{
  FILE *fp;
  int err;
  ...
  *binary = 1; /* Read it as binary */
  ...
  /* Don't cache the files while generating sums */
  fcntl( fileno( fp ), F_NOCACHE, 1);
#endif

  err = DIGEST_STREAM (fp, size, bin_result);
  if (err)
    {
      error (0, errno, "%s", filename);
      if (fp != stdin)
  ...
  do
    {
      ...
      if (! (split_3 (line, line_length, &hex_digest, &binary, &size, &filename)
      ...
      else
        {
          ...
          ok = digest_file (filename, &binary, &file_len, bin_buffer);
          ...
          else if (!normal_lines && file_len != size)
            {
		++n_size_mismatches;
		if (!status_only)
		  {
	  ...
	  if (n_size_mismatches > 0)
	    {
	      error (0, 0,
		     (ngettext
                      ("WARNING: %" PRIuMAX " file size did NOT match",
		       "WARNING: %" PRIuMAX " file sizes did NOT match",
                       select_plural (n_size_mismatches))),
		     n_size_mismatches);
	    }
        }
    }
...
main (int argc, char **argv)
{
  ...
  while ((opt = getopt_long (argc, argv, "bctwe", long_options, NULL)) != -1)
    switch (opt)
      {
      case 'b':
        binary = 1;
      ...
      case 'e':
        normal_lines = false;
        min_digest_line_length = MIN_DIGEST_LINE_LENGTH + 10 /* 64-bit size */;
        break;
  ...
  if (!O_BINARY && binary < 0)
    binary = 0;
#else
  /* All input and output should assume binary, even if it's text */
  binary = 1;
#endif
  ...
  if (files == 0)
    error(EXIT_FAILURE, errno, "malloc of %d bytes", (int)sizeof(char *) * end);
  for (i = 0; i < end; i++) {
    if (optind == argc && i == 0)
      files[i] = STRDUP("-\0");
    else
  ...
  for (i = 0; i < end; i++)
    {
...
#ifdef HAVE_STRING_H
      memset((void *)&statbuf, 0, sizeof(struct MYSTAT));
#endif

      if (strcmp(file, "-")) {
        ok = (MYSTAT(file, &statbuf) == 0);
        if ( ok != true ) {
          error(EXIT_FAILURE, errno, "error from stat(%s)", file);
      ...
      if (do_check)
      ...
      else
        {
          ...
          if (! digest_file (file, &file_is_binary, &size, bin_buffer))
            ok = false;
          else
            {
              size_t i;
	      ...
	      if (!normal_lines) {
		putchar (' ');
		printf ("%08lX", (unsigned long int)size);
		putchar (' ');
	      }
...
static void read_tree(const char *path, int *endpos, int *new_end, char ***files) {
  ...
  while((ent = readdir_ignoring_dot_and_dotdot(dirptr)) != NULL) {
