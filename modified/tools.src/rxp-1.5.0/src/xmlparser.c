/* 	$Id: xmlparser.c,v 1.135 2008/10/03 13:07:51 richard Exp $
*/

#define DEBUG_FSM 0

#ifndef lint
static char vcid[] = "$Id: xmlparser.c,v 1.135 2008/10/03 13:07:51 richard Exp $";
#endif /* lint */
...
static HashMapRetType check_id(const HashEntryStruct *id_entry, void *pp)
{
    ...
    if(!(long)hash_get_value(id_entry))
	validity_error(p,
...
static int check_attribute_token(Parser p, AttributeDefinition a, ElementDefinition e, const Char *value, int length, const char *message, int real_use)
{
	...
	if(!found)
	{
            long is_AT_id = (a->type == AT_id);
	    hash_set_value(id_entry, (void *)is_AT_id);
	    if(ParserGetFlag(p, XMLNamespaces))
		for(i=0; i<length; i++)
		    if(value[i] == ':')
		    {
	...
	{
	    long idinfo = (long)hash_get_value(id_entry);
	    if(idinfo & 1)
	    {
