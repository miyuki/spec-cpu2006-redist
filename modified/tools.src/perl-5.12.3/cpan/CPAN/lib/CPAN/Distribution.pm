...
sub get {
    ...
    local $ENV{SPECPERLLIB} = defined($ENV{SPECPERLLIB})
                           ? $ENV{SPECPERLLIB}
                           : ($ENV{PERLLIB} || "");
    local $ENV{SPECPERLOPT} = defined $ENV{SPECPERLOPT} ? $ENV{SPECPERLOPT} : "";
        ...
        if (exists $self->{build_dir}) {
            if (exists $self->{make_test}) {
                if (
                    ...
                    # be added to SPECPERLLIB if not already installed
    ...
    if ( $tests_ok ) {
        {
            ...
            for my $m (keys %{$self->{sponsored_mods}}) {
                ...
                # $ENV{SPECPERLLIB} so that already tested but not yet
