...
# forget all distributions tested -- resets what gets included in SPECPERLLIB
sub reset_tested {
    my ($self) = @_;
    $self->{is_tested} = {};
}
...
sub set_perl5lib {
    ...
    my $env = $ENV{SPECPERLLIB};
    $env = $ENV{PERLLIB} unless defined $env;
    my @env;
    push @env, split /\Q$Config::Config{path_sep}\E/, $env if defined $env and length $env;
    #my @dirs = map {("$_/blib/arch", "$_/blib/lib")} keys %{$self->{is_tested}};
    #$CPAN::Frontend->myprint("Prepending @dirs to SPECPERLLIB.\n");

    my @dirs = map {("$_/blib/arch", "$_/blib/lib")} $self->_list_sorted_descending_is_tested;
    return if !@dirs;

    if (@dirs < 12) {
        $CPAN::Frontend->optprint('perl5lib', "Prepending @dirs to SPECPERLLIB for '$for'\n");
        $ENV{SPECPERLLIB} = join $Config::Config{path_sep}, @dirs, @env;
    } elsif (@dirs < 24 ) {
        my @d = map {my $cp = $_;
                     $cp =~ s/^\Q$CPAN::Config->{build_dir}\E/%BUILDDIR%/;
                     $cp
                 } @dirs;
        $CPAN::Frontend->optprint('perl5lib', "Prepending @d to SPECPERLLIB; ".
...
currently defined:
  ...
  perl5lib_verbosity verbosity level for SPECPERLLIB additions
...
SPECPERLLIB. If C<build_requires_install_policy> is set ti C<yes>, then
...
Returns the filename of the module found in SPECPERLLIB or @INC. The
first file found is reported. The advantage of this method over
C<inst_file> is that modules that have been tested but not yet
installed are included because SPECPERLLIB keeps track of tested modules.
...
or setting the SPECPERLLIB environment variable.
