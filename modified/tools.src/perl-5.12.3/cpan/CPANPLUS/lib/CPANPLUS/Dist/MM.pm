...
sub prepare {
    ...
    RUN: {
        ...
        if( -e MAKEFILE->() && (-M MAKEFILE->() < -M $dir) && !$force ) {
            ...
            ### turn off our SPECPERLOPT so no modules from CPANPLUS::inc get
            ...
            ### to reset the @INC
            #local $ENV{SPECPERLOPT} = CPANPLUS::inc->original_perl5opt || ''; 
...
sub create {
    ...
    ### ->save_state, at which point we need to restore @INC/$SPECPERLLIB
