...
=cut
    ...
    sub _flush {
        ...
        for my $what (@$aref) {
            ...
            if( $what eq 'lib' ) {
                $ENV{SPECPERLLIB}  = $conf->_perl5lib || '';
                @INC            = @{$conf->_lib};
...
This means they get added to C<@INC> as well as C<$ENV{SPECPERLLIB}>.
...
sub _add_to_includepath {
    ...
    for my $lib (@$dirs) {
        ...
        #
        ### it will be complaining if $ENV{SPECPERLLIB] is not defined (yet).   
        local $^W;  
        $ENV{'SPECPERLLIB'} .= $s . $lib 
            unless $ENV{'SPECPERLLIB'} =~ qr|\Q$s$lib\E|;
    }
