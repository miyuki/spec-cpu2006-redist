...
C<SPECPERLLIB> environment variable do not appear in C<@INC>.

Because C<SPECPERLLIB> is often used during testing to add build
directories to C<@INC> C<Test::Harness> (actually
L<TAP::Parser::Source::Perl>) passes the names of any directories found
in C<SPECPERLLIB> as -I switches. The net effect of this is that
C<SPECPERLLIB> is honoured even in taint mode.
