...
# Determine the default @INC for this Perl
{
  ...
  sub _default_INC {
    ...
    local $ENV{SPECPERLLIB};  # this is not considered part of the default.
...
sub run_perl_command {
  ...
  local $ENV{SPECPERLLIB} = join $self->config('path_sep'), $self->_added_to_INC;
...
sub do_system {
  ...
  # Some systems proliferate huge SPECPERLLIBs, try to ameliorate:
  my %seen;
  my $sep = $self->config('path_sep');
  local $ENV{SPECPERLLIB} =
    ( !exists($ENV{SPECPERLLIB}) ? '' :
      length($ENV{SPECPERLLIB}) < 500
      ? $ENV{SPECPERLLIB}
      : join $sep, grep { ! $seen{$_}++ and -d $_ } split($sep, $ENV{SPECPERLLIB})
    );
