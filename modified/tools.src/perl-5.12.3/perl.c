...
S_parse_body(pTHX_ char **env, XSINIT_t xsinit)
{
  ...
  switch_end:
    ...
    if (
        ...
        !PL_tainting &&
#endif
#if 0 /* Never honor PERL5OPT */
	(s = PerlEnv_getenv("PERL5OPT")))
#else
	(s = PerlEnv_getenv("SPECPERLOPT")))
#endif
    {
	while (isSPACE(*s))
	    s++;
...
#  endif
 ...
 #endif
	    PerlIO_printf(PerlIO_stdout(),
		"\nThis is specperl "	STRINGIFY(PERL_REVISION)
...
/* Adjust verbose output as in the perl that ships with the DG/UX OS from EMC */
	...
	if (LOCAL_PATCH_COUNT > 0)
	    PerlIO_printf(PerlIO_stdout(),
			  ...
			  "see specperl -V for more detail)",
			  LOCAL_PATCH_COUNT,
...
this system using \"specperldoc perl\".  If you have access to the\n\
...
S_init_perllib(pTHX)
{
    ...
    if (!PL_tainting) {
#ifndef VMS
#if 0 /* Never honor PERL5LIB */
	perl5lib = PerlEnv_getenv("PERL5LIB");
#else
	perl5lib = PerlEnv_getenv("SPECPERLLIB");
#endif
/*
...
#else
	if (my_trnlnm("SPECPERLLIB",buf,0))
	    do {
		incpush_use_sep(buf, 0, INCPUSH_ADD_SUB_DIRS);
	    } while (my_trnlnm("SPECPERLLIB",buf,++idx));
#endif
#if 0 /* Or PERLLIB */
	else {
...
#if 0 /* No, don't use the undocumented ~-expanded dirs */
