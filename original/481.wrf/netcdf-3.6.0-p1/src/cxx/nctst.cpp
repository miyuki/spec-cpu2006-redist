...
void gen(const char* path, int version)		// Generate a netCDF file
{
    NcFile::FileFormat format;

    if (version == 1)
	format = NcFile::Classic;
    else if (version == 2)
	format = NcFile::Offset64Bits;
    else
    {
	cerr << "Bad format version " << version << "\n";
	return;
    }

    NcFile nc(path, NcFile::Replace, NULL, 0, NcFile::Classic); // Create, leave in define mode
...
main( void )	// test new netCDF interface
{
    ...
    gen(path, 1);		// create a netCDF file
    dump(path);		// print what's in it

    gen(path, 2);		// create a netCDF file
    dump(path);		// print what's in it
